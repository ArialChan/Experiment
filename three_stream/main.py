...
########### heechan's experiment

# test_path = "./meta_data/cider_jsons/gts_test.pkl"
...
# Comet Experiment
experiment = Experiment(api_key="V3GdakcYxDRFXAjRFLuS3LVP5",
                        project_name="HACA_Model", workspace="chay219")
...
def test_model(model, mode, beam_search):
    ...
    ###
    gts = {}
    with open('./meta_data/cider_jsons/gts_train.pkl', "rb") as file:
        gts1 = pickle.load(file)
        # print("gts1 ", len(gts1['annotations']))
    with open('./meta_data/cider_jsons/gts_valid.pkl', "rb") as file:
        gts2 = pickle.load(file)
        gts1['annotations'].extend(gts2['annotations'])
        # print("gts2 ", len(gts2['annotations']))
        # print("gts1 ", len(gts1['annotations']))
    with open('./meta_data/cider_jsons/gts_test.pkl', "rb") as file:
        gts3 = pickle.load(file)
        gts1['annotations'].extend(gts3['annotations'])
        # print("gts3 ", len(gts3['annotations']))
        # print("gts1 ", len(gts1['annotations']))
    with open('./meta_data/cider_jsons/gts_luna.pkl', "rb") as file:
        gts4 = pickle.load(file)
        gts1['annotations'].extend(gts4['annotations'])
    gts = gts1
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "visual_input_size" : 2048, # 원래 2048
    ...
    if "{}.pt".format(args.model) in os.listdir('models'):
        print("Loading Pre-Trained Model first!!!!")
