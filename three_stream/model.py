...
class decoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, isglobal, encoder_output_size, device, embedding_size=None, vocab_size=None, global_output_size=None):
        ...
        # Attention matrices used to calculate pwc context at time t
        self.pwc_attention_weight_1 = nn.Linear(self.hidden_size, self.context_size, bias=False)
        self.pwc_attention_weight_2 = nn.Linear(encoder_output_size["pwc"], self.context_size, bias=False)
        self.pwc_attention_weight_3 = nn.Linear(self.context_size, 1, bias=False)
        self.pwc_enc_to_dec_mat = nn.Linear(encoder_output_size["pwc"], self.context_size, bias=True)
    ...
    def forward(self, input, encoder_outputs, hidden_states, cell_state, context):
        ...
        attention_weights = [self.pwc_attention_weight_1, self.pwc_attention_weight_2, self.pwc_attention_weight_3]
        pwc_context = self.get_context(torch.transpose(hidden_state, 0, 1), encoder_outputs["pwc"], attention_weights)
        pwc_context = torch.unsqueeze(self.pwc_enc_to_dec_mat(pwc_context), dim=1)
        ...
        tanh = nn.Tanh()
        all_context = torch.cat([visual_context, hidden_context, audio_context, pwc_context], dim=1)
...
class HACAModel(nn.Module):
    ...
    def __init__(self, config, device, batch_size, max_caption_length):
        ...
        self.pwc_encoder = encoder(config["visual_input_size"], config["visual_hidden_size"], config["chunk_size"]["visual"], device)

        # Decoders (Global & Local)
        encoder_output_size = {"audio" : 2*config["audio_hidden_size"]["low"], "visual" : 2*config["visual_hidden_size"]["low"], "pwc" : 2*config["visual_hidden_size"]["low"]}
        # Local Decoder
        ...
        encoder_output_size = {"audio" : config["audio_hidden_size"]["high"], "visual" : config["visual_hidden_size"]["high"],"pwc" : config["visual_hidden_size"]["high"]}
    ...
    def forward(self, input, ss_epsilon, beam_search=False, beam_width=-1):
        ...
        pwc_outputs_low, pwc_outputs_high = self.pwc_encoder(input["pwc_features"])

        # decoder steps
        local_encoder_outputs = {"audio" : audio_outputs_low, "visual" : visual_outputs_low, "pwc": pwc_outputs_low}
        global_encoder_outputs = {"audio" : audio_outputs_high, "visual" : visual_outputs_high, "pwc" : pwc_outputs_high}
